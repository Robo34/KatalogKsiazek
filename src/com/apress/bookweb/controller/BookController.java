...
public class BookController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action != null) {
			...
			findAllCovers(request, response);
		}
	...
	private void findAllCovers(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		try {
			BookDAO bookDao = new BookDAOImpl();
			List<Book> bookList = bookDao.findAllBooks();
			List<Book> coverBooks = new ArrayList<>();
			for (Book cB : bookList){
//				coverBook // cB wyselekcjonowac pozcyje z okladkami i przypisac do coverBook
				if (cB.getCoverTitle() != null && !"".equals(cB.getCoverTitle())){
					coverBooks.add(cB);
				}
						
			}
			
			request.setAttribute("bookList", coverBooks);
